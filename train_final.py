...
import data_manager
from data_loader import Data_Loader, Data_Loader_unlabel, Data_Loader_unlabel_pet, Data_Loader_unlabel_final
...
import models
from utils import Logger, set_random_seed, AverageMeter, get_pre_info, get_specific_label_info, get_unlabel_best_model
...
parser.add_argument("--trainset", default="/xxx/dataset/multinli/train_select_10.csv", type=str, required=False, help="trainset.")
parser.add_argument("--testset", default="/xxx/dataset/multinli/test_matched.csv", type=str, required=False, help="trainset.")
parser.add_argument("--unlabelset", default="/xxx/dataset/multinli/unlabeled_select_10.csv", type=str, required=False, help="trainset.")
parser.add_argument("--pretrainset", default="/xxx/dataset/multinli/train_select_10.csv", type=str, required=False, help="trainset.")
...
parser.add_argument('--unlable_loader', default='Data_Loader_unlabel_final', type=str, help="unlabel dataset_loader")

parser.add_argument("--model", default='transformer', type=str, help="model.")
...
parser.add_argument("--num_train_steps", default=5000, type=int, help="Total number of training epochs to perform.")
...
parser.add_argument("--unlabel_tag", default='', type=str, help="tag for save.")
parser.add_argument("--retrain_rounds", default=3, type=int, help="tag for save.")
parser.add_argument("--tag", default='', type=str, help="tag for save.")
...
def main():
    ...
    args.output_dir = os.path.join(args.output_dir, 'transformer_pre_fc_tanh_init', args.dataset, args.tag)
    args.work_dir = args.output_dir.replace('/' + args.output_dir.split('/')[-1], '')
    ...
    if not args.eval_only:
        args.model_dict_test = get_unlabel_best_model(args.work_dir, args.unlabel_tag, args.retrain_rounds)
        if args.retrain_rounds != 0:
            args.pattern_id = int(args.model_dict_test[args.model_dict_test.index('pattern_'): ].split('_')[1])
        else:
            args.pattern_id = int(args.model_dict_test[args.model_dict_test.index('pattern_') + 8: args.model_dict_test.index('pattern_') + 9])
        print('best pattern_id: %i'%args.pattern_id)
    ...
    if (not args.eval_only):
        ...
        unlabel_loader = eval(args.unlable_loader)(train_dataset, unlabel_dataset, args.work_dir, args.unlabel_tag, args.retrain_rounds, 
    ...
    if args.eval_only:
        # Does predictions.
        print("\n=====start predicting=====")
        acc, test_loss = evaluate(model, criterion, testloader, use_gpu, args, save_name=None)
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        ...
        # acc_train, train_loss = evaluate(model, criterion, trainloader, use_gpu, args, save_name=None)
...
def pretrain_model(model, lmtrainloader, tokenizer, config, pin_memory, criterion, use_gpu, args):
    ...
    for pretrain_epoch in range(args.num_pretrain_epochs):
        ...
        epoch_time = time.time()
        args.lm_traing = False
        train(model, optimizer_pretrain, lr_scheduler, criterion, pretrainloader, lmtrainloader, tokenizer, pretrain_epoch, args, use_gpu)
        epoch_time = time.time() - epoch_time
        print("epoch time footprint: %d hour %d min %d sec" %
            (epoch_time // 3600, (epoch_time % 3600) // 60, epoch_time % 60))
